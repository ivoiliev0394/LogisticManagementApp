    ...
    public class AdminCrudService : IAdminCrudService
    {
        ...
        public AdminEntityFormViewModel GetCreateForm(string entityName)
        {
            ...
            return new AdminEntityFormViewModel
            {
                ...
                IsEdit = false,
                Fields = GetBrowsableProperties(entityType)
                    .Select(p => BuildFieldViewModel(entityType, p, GetCreateDefaultValue(entityType, p), isEdit: false))
                    .ToList()
            };
        }
        ...
        public string CreateEntity(string entityName, IDictionary<string, string?> values)
        {
            ...
            var normalizedValues = PrepareAutoGeneratedValues(entityType, values, isEdit: false);
            normalizedValues = PrepareGeneratedGuidValues(entityType, normalizedValues, isEdit: false);
            ApplyValues(entityType, entity, normalizedValues, isEdit: false);
        ...
        public bool UpdateEntity(string entityName, string key, IDictionary<string, string?> values)
        {
            ...
            var normalizedValues = PrepareAutoGeneratedValues(entityType, values, isEdit: true);
            normalizedValues = PrepareGeneratedGuidValues(entityType, normalizedValues, isEdit: true);
            ApplyValues(entityType, entity, normalizedValues, isEdit: true);
        ...
        private object? GetCreateDefaultValue(IEntityType entityType, IProperty property)
        {
            if (IsAutoGeneratedIdentifierProperty(property))
                return GenerateAutoGeneratedValue(entityType, property);

            return null;
        }

        private Dictionary<string, string?> PrepareAutoGeneratedValues(IEntityType entityType, IDictionary<string, string?> values, bool isEdit)
        {
            ...
            foreach (var property in GetBrowsableProperties(entityType))
            {
                ...
                if (isEdit)
                {
                    normalizedValues.Remove(property.Name);
                    continue;
                }

                if (normalizedValues.TryGetValue(property.Name, out var existingValue) && !string.IsNullOrWhiteSpace(existingValue))
                    continue;

                var generatedValue = GenerateAutoGeneratedValue(entityType, property);
                if (!string.IsNullOrWhiteSpace(generatedValue))
                {
                    normalizedValues[property.Name] = generatedValue;
                }
            }
        ...
        private Dictionary<string, string?> PrepareGeneratedGuidValues(IEntityType entityType, IDictionary<string, string?> values, bool isEdit)
        {
            var normalizedValues = new Dictionary<string, string?>(values, StringComparer.OrdinalIgnoreCase);

            foreach (var property in GetBrowsableProperties(entityType))
            {
                if (!ShouldAutoGenerateGuid(property))
                    continue;
                ...
                if (normalizedValues.TryGetValue(property.Name, out var existingValue)
                    && Guid.TryParse(existingValue, out var existingGuid)
                    && existingGuid != Guid.Empty)
                {
                    continue;
                }

                normalizedValues[property.Name] = Guid.NewGuid().ToString();
            }

            return normalizedValues;
        }

        private static bool ShouldAutoGenerateGuid(IProperty property)
        {
            ...
            return property.IsPrimaryKey() || string.Equals(property.Name, "Id", StringComparison.OrdinalIgnoreCase);
        }

        private string? GenerateAutoGeneratedValue(IEntityType entityType, IProperty property)
        {
            var clrType = Nullable.GetUnderlyingType(property.ClrType) ?? property.ClrType;
            if (clrType != typeof(string))
                return null;

            var prefix = GetAutoGeneratedPrefix(property.Name);
            if (string.IsNullOrWhiteSpace(prefix))
                return null;

            var yearPrefix = $"{prefix}-{DateTime.UtcNow.Year}-";
            var existingValues = QueryEntityObjects(entityType)
                .Select(entity => GetPropertyValue(entity, property.Name) as string)
                .Where(value => !string.IsNullOrWhiteSpace(value) && value!.StartsWith(yearPrefix, StringComparison.OrdinalIgnoreCase))
                .Select(value => value!)
                .ToList();

            var nextNumber = existingValues
                .Select(value => value[yearPrefix.Length..])
        ...
        private static bool IsAutoGeneratedIdentifierProperty(IProperty property)
        {
            var clrType = Nullable.GetUnderlyingType(property.ClrType) ?? property.ClrType;
            if (clrType != typeof(string))
                return false;
            ...
            return GetAutoGeneratedPrefix(property.Name) != null;
        }

        private static string? GetAutoGeneratedPrefix(string propertyName)
        {
            return propertyName switch
            {
                ...
                "SealNumber" => "CSL",
                "VoyageNumber" => "VOY",
                _ => TryBuildGenericPrefix(propertyName)
            };
        }

        private static string? TryBuildGenericPrefix(string propertyName)
        {
            var baseName = propertyName;
            foreach (var suffix in new[] { "Code", "Number", "No" })
            {
                if (baseName.EndsWith(suffix, StringComparison.OrdinalIgnoreCase) && baseName.Length > suffix.Length)
                {
            ...
            if (string.Equals(baseName, propertyName, StringComparison.OrdinalIgnoreCase))
                return null;

            var parts = Regex.Matches(baseName, @"[A-Z][a-z0-9]*|[a-z0-9]+")
            ...
            var prefix = string.Concat(parts.Select(part => char.ToUpperInvariant(part[0])));
            if (prefix.Length >= 3)
                return prefix[..3];

            return prefix.PadRight(3, prefix[^1]);
        }
        ...
        private AdminEntityFieldViewModel BuildFieldViewModel(IEntityType entityType, IProperty property, object? value, bool isEdit)
        {
            ...
            var enumOptions = isEnum
                ? GetEnumOptions(clrType)
                : new List<AdminForeignKeyOptionViewModel>();
            ...
            return new AdminEntityFieldViewModel
            {
                ...
                IsReadOnly = (property.ValueGenerated != ValueGenerated.Never && !isEdit) || IsAutoGeneratedIdentifierProperty(property),
                IsHidden = ShouldHideInAdminForm(property, isEdit),
                IsForeignKey = isForeignKey,
                IsEnum = isEnum,
                IsDecimalNumber = clrType == typeof(decimal) || clrType == typeof(double) || clrType == typeof(float),
                UseForeignKeyDropdown = foreignKeyOptions.Count > 0,
                UseEnumDropdown = !isForeignKey && enumOptions.Count > 0,
                ForeignEntityName = foreignKey?.PrincipalEntityType?.ClrType?.Name,
                EnumTypeName = isEnum ? clrType.Name : null,
        ...
        private static bool ShouldHideInAdminForm(IProperty property, bool isEdit)
        {
            var clrType = Nullable.GetUnderlyingType(property.ClrType) ?? property.ClrType;

            if (clrType == typeof(Guid) && (property.IsPrimaryKey() || string.Equals(property.Name, "Id", StringComparison.OrdinalIgnoreCase)))
                return true;

            return false;
        }

        private static List<AdminForeignKeyOptionViewModel> GetEnumOptions(Type enumType)
        {
            return Enum.GetNames(enumType)
                .Select(name => new AdminForeignKeyOptionViewModel
                {
                    Value = name,
                    Text = SplitPascalCase(name)
                })
                .ToList();
        }
        ...
        private static object? ConvertFromString(string? rawValue, Type targetType)
        {
            ...
            rawValue = NormalizeNumericValue(rawValue, effectiveType);
        ...
        private static string? NormalizeNumericValue(string? rawValue, Type effectiveType)
        {
            if (string.IsNullOrWhiteSpace(rawValue))
                return rawValue;
